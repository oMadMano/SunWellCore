	...
	if (!preventionOnly)
	{
		if (unitflag & UNIT_FLAG_STUNNED)
		{
			...
			if (usableInStun)
			{
				...
				// Pain Suppression, can be used while trapped
				if (m_spellInfo->Id == 33206) // Note: Only if player has Glyph of Pain Suppression active
					mask |= 1 << MECHANIC_FREEZE | 1 << MECHANIC_HORROR;
